                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  if (thread_2D_pos.x >= numCols || thread_2D_pos.y >= numRows)
    return;
  float sum = 0.0f;
  int pos = 0, x,y; //position in the filter
  for (int dy=-filterWidth/2; dy<filterWidth-filterWidth/2; dy++){
    y = thread_2D_pos.y + dy;
    if (y<0) y = 0;
    else if (y>=numRows) y=numRows-1;
    for (int dx=-filterWidth/2; dx<filterWidth-filterWidth/2; dx++){
      x = thread_2D_pos.x + dx;
      if (x<0) x = 0;
      else if (x>=numCols) x = numCols-1;

      sum +=filter[pos] * (float) inputChannel[y*numCols+x];
      pos ++;
    }
  }
                                ...
                                const float* const h_filter, const size_t filterWidth)
{
  ...
  checkCudaErrors(cudaMalloc(&d_blue,  sizeof(unsigned char) * numRowsImage * numRowsImage));
